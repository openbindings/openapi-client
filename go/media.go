...
func urlencodedArrayNeedsPropertyMedia(schema *openapi3.Schema, is30 bool) bool {
	...
	if !ok {
		...
		// 3.1/3.2 lines is the "determines no default" cell and is reported
		// by contentPropertyDeterminesNoDefault instead; a choice applicator
		// that supplies no single resolved member is left with no resolved
		// member declaration under Section 5.2, and the ordinary refusal
		// stands.
		return false
	}
