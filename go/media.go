...
func planRequestBodiesFor(doc *openapi3.T, op *openapi3.Operation, bindingSpec string) ([]*bodyPlan, error) {
	...
	if len(candidates) == 0 {
		...
		return nil, fmt.Errorf("request body declares only media types outside the families openapi@1 defines a request carriage for (declared: %s)", strings.Join(declared, ", "))
	}
...
// flag lets a JSON null value elide the optional part.
