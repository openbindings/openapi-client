...
func applyResponseMechanics(request *http.Request, response *http.Response, document *openapi3.T, operation *openapi3.Operation, profile string, codecs map[string]ContentDecoder, bufferEventStreams bool) (*http.Response, error) {
	...
	buffer := bufferEventStreams || rawCoding != "" || request.Method == http.MethodHead
	if !buffer {
		return response, nil
	}
	...
	if rawCoding != "" {
		if governing == nil {
			return nil, fmt.Errorf("coded response has no governing Response Object")
		}
		header := responseHeader(governing.response, "Content-Encoding")
		if header == nil {
			return nil, fmt.Errorf("actual response Content-Encoding has no governing Header Object")
		}
		edition := ""
		if document != nil {
			edition = document.OpenAPI
		}
		if !schemaAdmitsHeaderValue(header.Schema, rawCoding, edition) {
			return nil, fmt.Errorf("actual response Content-Encoding is not admitted by its governing Header Object")
		}
