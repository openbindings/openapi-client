...
// delivery-unit bound — executionArgs.MaxDeliveryUnitBytes does not
// apply here.
...
func isSSEContentType(contentType string) bool {
	return isSSEContentTypeFor(contentType, profileRoutedCoordinate)
}
...
// server-sent events processing model (openapi@1 §8,
...
// transition: closeOutputBoundary when the body is exhausted cleanly,
// failExecution(ERR_STREAM_ERROR) on a scan failure (abnormal termination is a
...
func streamSSE(ctx context.Context, resp *http.Response, args *executionArgs, site HookSite, inv executionHandle[any, any]) {
	...
	for scanner.Scan() {
		...
		// executionArgs.MaxDeliveryUnitBytes (default 10 MiB).
