...
func unionTypeEmission(t *testing.T, doc *openapi3.T, media string, body *openapi3.MediaType, value any) string {
	...
	if media == "application/x-www-form-urlencoded" {
		encoded, err := buildURLEncodedBodyForRevision(doc, body, map[string]any{"p": value}, profileFullCoordinate)
		if err != nil {
			return "error"
		}
		if encoded == "" {
	...
	reader, contentType, err := buildMultipartBodyForRevision(doc, body, map[string]any{"p": value}, profileFullCoordinate)
	if err != nil {
		return "error"
	}
