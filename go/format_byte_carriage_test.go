...
func formByteCarriageEmission(t *testing.T, doc *openapi3.T, media *openapi3.MediaType, plan *bodyPlan, c formByteCarriageCase) string {
	...
	case "urlencoded":
		encoded, err := buildURLEncodedBodyForRevision(doc, media, map[string]any{c.PropertyName: c.Value}, profileFullCoordinate)
		if err != nil {
			return "error"
		}
		if encoded == "" {
	...
	reader, contentType, err := buildMultipartBodyForRevision(doc, media, map[string]any{c.PropertyName: c.Value}, profileFullCoordinate)
	if err != nil {
		return "error"
	}
