...
const (
	...
	serverVariableInvalidMarker = "x-openapi-client-internal-server-invalid"
	referringSecurityMarker     = "x-openapi-client-internal-referring-security-schemes"
)
...
// markReferringSecuritySchemes carries the Security Scheme component scope of
// an externally referenced Path Item through the typed loader. OAS 3.0 allows
// the referenced operation's Security Requirement names to resolve against
// either the entry or referring document by explicit consumer choice; the
...
func (n *rawRefSiblingNormalizer) markReferringSecuritySchemes(data []byte, requested, retrieval *url.URL) ([]byte, error) {
	...
	for _, target := range n.targetsForResources(requested, retrieval) {
		...
		pathItem, _ := node.(map[string]any)
		if !ok || pathItem == nil {
			continue
		}
		if _, collision := pathItem[referringSecurityMarker]; collision {
			return nil, fmt.Errorf("OpenAPI path item uses processor-reserved extension %q", referringSecurityMarker)
		}
		pathItem[referringSecurityMarker] = schemes
		changed = true
	}
	if !changed {
		return data, nil
	...
	if err != nil {
		return nil, fmt.Errorf("marshal referring security scope: %w", err)
	}
	return encoded, nil
}
...
func (n *rawRefSiblingNormalizer) normalizeTarget(value any, kind rawRefTargetKind, semantics rawRefSemantics, base *url.URL) (bool, error) {
	...
	case rawPathItemTarget:
		...
		for _, method := range httpMethods {
			...
			responseChanged, err := n.normalizeResponseMembers(operation["responses"], semantics, base)
			if err != nil {
				return false, err
			}
			changed = changed || responseChanged
...
func (n *rawRefSiblingNormalizer) normalizeResponseMembers(value any, semantics rawRefSemantics, base *url.URL) (bool, error) {
	...
	for key, raw := range responses {
		...
		if _, ok := raw.(map[string]any); !ok {
			// The admitted key retains precedence, but the invalid Response
			// projection has no readable body/header declaration. An empty typed
			// Response is the exact private execution image for that state.
			responses[key] = map[string]any{}
			changed = true
			continue
		}
		itemChanged, err := n.normalizeTarget(raw, rawResponseTarget, semantics, base)
		if err != nil {
			return false, err
		}
...
func normalizeRawResponseObject(response map[string]any) bool {
	changed := false
	if raw, present := response["description"]; present {
...
func markRawServers(value any, base *url.URL) bool {
	...
	for _, rawServer := range servers {
		...
		for _, rawVariable := range variables {
			...
			if rawDefault, present := variable["default"]; present {
				variable[serverVariableDefaultMarker] = true
				changed = true
				if _, valid := rawDefault.(string); !valid {
					// Preserve the coverage fact while giving kin-openapi a private
					// typed-model placeholder. eligibleServers sees the marker and
					// confines the invalid Server alternative.
					variable[serverVariableInvalidMarker] = true
					variable["default"] = ""
				}
			}
