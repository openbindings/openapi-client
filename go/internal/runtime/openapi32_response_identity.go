...
func (a *Artifact) materializeOpenAPI32ResponseTarget(target *OperationTarget) (*OperationTarget, error) {
	...
	for key, raw := range rawResponses {
		...
		if err != nil {
			...
			// Object is unavailable. Represent that invalid subordinate
			// projection as an empty response rather than removing the target or
			// falling through to a less-specific key.
			responses.Set(key, &openapi3.ResponseRef{Value: &openapi3.Response{}})
			continue
		}
...
func (o *OpenAPI32Overlay) materializeOpenAPI32Response(key string, node openAPI32RawNode) (*openapi3.Response, []OpenAPI32ResponseMediaExclusion, error) {
	...
	if object == nil {
		return &openapi3.Response{}, nil, nil
	}
	// Invalid documentary and container members are confined to their smallest
	// projection. Build the typed invocation image from every surviving
	// wire-relevant sibling; coverage tooling retains the raw source separately.
	typedObject := make(map[string]any, len(object))
	for name, value := range object {
		typedObject[name] = value
	}
	for _, field := range []string{"description", "summary"} {
		if value, present := typedObject[field]; present {
			if _, ok := value.(string); !ok {
				delete(typedObject, field)
			}
		}
	}
	...
	delete(typedObject, "headers")
	delete(typedObject, "links")
	delete(typedObject, "content")
	encoded, err := json.Marshal(typedObject)
	if err != nil {
		return nil, nil, err
	}
	...
	if rawHeaders, present := object["headers"].(map[string]any); present {
		...
		for name, raw := range rawHeaders {
			...
			if resolveErr != nil {
				if rawValueContainsReference(raw) {
	...
	if rawLinks, present := object["links"].(map[string]any); present {
		for name, raw := range rawLinks {
			...
			if _, resolveErr := o.resolveOpenAPI32ObjectNode(
				openAPI32RawNode{value: raw, resource: node.resource},
...
func rawValueContainsReference(value any) bool {
	...
	case map[string]any:
		...
		for _, child := range typed {
			if rawValueContainsReference(child) {
				return true
			}
		}
	case []any:
