	...
	for _, parameter := range parameters {
		...
		case Swagger20ParameterHeader:
			for _, contribution := range contributions {
				...
				if strings.EqualFold(parameter.name, "Cookie") && !swagger20CookieString(contribution.value) {
					return swagger20RoutedInput{}, fmt.Errorf("header parameter %q is not a complete RFC 6265 Cookie field value", parameter.name)
				}
...
func swagger20CookieString(value string) bool {
	...
	for _, pair := range pairs {
		...
		if equals <= 0 || !swagger20HTTPFieldName(pair[:equals]) {
			return false
		}
		cookieValue := pair[equals+1:]
		if len(cookieValue) >= 2 && cookieValue[0] == '"' && cookieValue[len(cookieValue)-1] == '"' {
			cookieValue = cookieValue[1 : len(cookieValue)-1]
		}
		if !validRFC6265CookieValue(cookieValue) {
			return false
		}
	}
	return true
