...
func (r abstractInputRoutes) transformExpression() string {
	return r.transformExpressionFor(profileRoutedCoordinate)
}
...
func (r abstractInputRoutes) transformExpressionFor(bindingSpec string) string {
	...
	return fmt.Sprintf(`[{"$openapi":"%s","value":$,"parameters":%s,"body":%s}]`,
...
func parseRoutedEnvelope(input any) (*routedEnvelope, error) {
	return parseRoutedEnvelopeFor(input, profileRoutedCoordinate)
}

func parseRoutedEnvelopeFor(input any, bindingSpec string) (*routedEnvelope, error) {
	return parseRoutedEnvelopeWithKey(input, "$openapi", bindingSpec)
}

func parseRoutedEnvelopeWithKey(input any, key, markerValue string) (*routedEnvelope, error) {
	...
	marker, marked := envelope[key]
	if !marked {
		return nil, fmt.Errorf("routed input array item requires %s marker", key)
	}
	if len(envelope) != 4 || envelope["value"] == nil || envelope["parameters"] == nil || envelope["body"] == nil {
		return nil, fmt.Errorf("routed input array item must contain exactly %s, value, parameters, and body", key)
	}
	if marker != markerValue {
		return nil, fmt.Errorf("routed input has invalid %s marker %v", key, marker)
	}
...
func routeEnvelope(params openapi3.Parameters, envelope *routedEnvelope, pathTemplate string, plan *bodyPlan) (*routedInput, error) {
	return routeEnvelopeFor(params, envelope, pathTemplate, plan, profileRoutedCoordinate)
}
...
func routeEnvelopeWithParameterOptions(params openapi3.Parameters, envelope *routedEnvelope, pathTemplate string, plan *bodyPlan, bindingSpec string, options parameterSerializationOptions) (*routedInput, error) {
	r := &routedInput{
		...
		bodyPresent:  envelope.bodyPresent,
