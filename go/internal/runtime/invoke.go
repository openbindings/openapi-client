...
func runBinding(ctx context.Context, client *http.Client, args *executionArgs, inv executionHandle[any, any], artifact *Artifact) {
	...
	target = targetForImplicitConnectionScope(target, args.Context)
	...
	op, err = operationWithTraceSafeSecurity(doc, op, method)
	if err != nil {
	...
	if member := styleLaneUndefinedExpansionParamFor(params, args.Source.Capability, isOpenAPI30(doc.OpenAPI)); member != "" {
		inv.failExecution(&ExecutionError{
			Code:    CodeRefused,
			Message: fmt.Sprintf("operation parameter member %q has no declaration-defined style expansion", member),
		})
		return
	}
	...
	if err := securitySelectionConfigurationError(doc, op, baseURL, params, args.Context); err != nil {
		inv.failExecution(&ExecutionError{Code: CodeRefused, Message: err.Error()})
		return
	}
	...
	mediaDetails, mediaRequirementErr := requiredRequestMediaContextWithPlans(doc, op, args.Source.Capability, args.Context, openAPI32Plans)
	...
	if len(plans) == 0 {
		...
		for _, candidate := range selectedPlans {
			...
			candidate.requestCharacterEncodings = args.RequestCharacterEncodings
	...
	if rawQuery == "" && len(routed.populated[ParameterInQueryString]) > 0 {
		completedURL.ForceQuery = true
	}
	...
	for _, selected := range selectedSchemes {
		...
		if err := handler(req, SecurityHandlerContext{
			SchemeName: selected.name,
			Scheme:     selected.scheme,
			Operation: OperationInfo{
				Ref: target.Ref, Path: target.Path, Method: Method(target.Method),
				WireMethod: target.WireMethod(), Additional: target.Additional,
	...
	client = clientWithCredentialSafeRedirects(client, credentialOwnership)
	...
	if hasMediaFidelity(args.Source.Capability) {
		...
		if revision3ContentTypeErr == nil && actualContentType == "" {
...
func clientWithCredentialSafeRedirects(client *http.Client, ownership []credentialPlacement) *http.Client {
	if client == nil {
		return client
	}
	clone := *client
	...
	clone.CheckRedirect = func(next *http.Request, via []*http.Request) error {
		if len(via) > 0 {
			prior := via[len(via)-1]
			if next.Method != prior.Method || (prior.Body != nil && prior.Body != http.NoBody && (next.Body == nil || next.Body == http.NoBody)) {
				return http.ErrUseLastResponse
			}
		}
		if len(via) > 0 && !sameHTTPOrigin(via[0].URL, next.URL) {
			// Cookie is origin-sensitive regardless of whether its source was a
			// selected credential, a declared cookie parameter, or a raw caller
			// contribution.
			next.Header.Del("Cookie")
			query := next.URL.Query()
			for _, placement := range ownership {
				switch placement.channel {
				case "header":
					next.Header.Del(placement.name)
					if strings.EqualFold(placement.name, "Cookie") {
						next.Header.Del("Cookie")
					}
				case "cookie":
					next.Header.Del("Cookie")
...
func operationWithTraceSafeSecurity(doc *openapi3.T, operation *openapi3.Operation, method string) (*openapi3.Operation, error) {
	if !strings.EqualFold(method, http.MethodTrace) || operation == nil {
		return operation, nil
	}
	requirements := effectiveSecurityRequirements(doc, operation)
	if requirements == nil || len(*requirements) == 0 {
		return operation, nil
	}
	excluded := map[int]bool{}
	usable := 0
	for index, alternative := range *requirements {
		safe := len(alternative) == 0
		if !safe {
			safe = true
			for name := range alternative {
				scheme, present := securityScheme(doc, name)
				if !present || scheme.Type != "mutualTLS" {
					safe = false
					break
				}
			}
	...
	if usable == 0 {
		return nil, fmt.Errorf("TRACE has no security alternative that avoids emitting credentials or cookies")
	}
	clone := *operation
	clone.Extensions = make(map[string]any, len(operation.Extensions)+1)
	for name, value := range operation.Extensions {
		clone.Extensions[name] = value
	}
	clone.Extensions[traceExcludedSecurityAlternatives] = excluded
	return &clone, nil
}

const traceExcludedSecurityAlternatives = "x-ob-internal-trace-excluded-security-alternatives"

func traceExcludesSecurityAlternative(operation *openapi3.Operation, index int) bool {
	if operation == nil || operation.Extensions == nil {
		return false
	}
	excluded, _ := operation.Extensions[traceExcludedSecurityAlternatives].(map[int]bool)
...
func requiredRequestMediaContext(doc *openapi3.T, op *openapi3.Operation, bindingSpec string, bindCtx map[string]any) (*Prerequisites, error) {
	return requiredRequestMediaContextWithPlans(doc, op, bindingSpec, bindCtx, nil)
}

func requiredRequestMediaContextWithPlans(doc *openapi3.T, op *openapi3.Operation, bindingSpec string, bindCtx map[string]any, plans []*bodyPlan) (*Prerequisites, error) {
	...
	if len(plans) == 0 {
		return nil, fmt.Errorf("required OpenAPI request body declares no admissible media alternative")
	}
...
func decodeByContentTypeFor(contentType, bindingSpec string) outputDecoder {
	return decodeByContentTypeWith(contentType, bindingSpec, nil)
}

func decodeByContentTypeWith(contentType, bindingSpec string, characterDecoders map[string]CharacterDecoder) outputDecoder {
	...
	return func(_ HookSite, raw RawResult) (any, error) {
		...
		return decodeTextLaneWith(contentType, raw.Body, bindingSpec, characterDecoders)
	}
}
...
func decodeTextLaneFor(contentType string, body []byte, bindingSpec string) (any, error) {
	return decodeTextLaneWith(contentType, body, bindingSpec, nil)
}

func decodeTextLaneWith(contentType string, body []byte, bindingSpec string, decoders map[string]CharacterDecoder) (any, error) {
	charset := "utf-8"
	if contentType != "" {
	...
	default:
		decoder := decoders[strings.ToLower(charset)]
		if decoder == nil {
			return nil, &ExecutionError{
				Code:    CodeResponseError,
				Message: fmt.Sprintf("response declares charset %q, which this implementation cannot decode; install a response character decoder", charset),
			}
		}
		value, err := decoder(body)
		if err != nil {
			return nil, &ExecutionError{Code: CodeResponseError, Message: fmt.Sprintf("response character-data charset %q failed: %v", charset, err), Cause: err}
		}
		return value, nil
	}
}
...
func requiredContext(doc *openapi3.T, op *openapi3.Operation, bindCtx map[string]any, baseURL string, params openapi3.Parameters) *Prerequisites {
	...
	plans, _ = securityPlansForSelection(plans, bindCtx)
	if len(plans) == 0 {
		return nil
	}
...
func configuredSecurityAlternative(bindCtx map[string]any) (int, bool, error) {
	cfg := contextConfiguration(bindCtx)
	if cfg == nil {
		return 0, false, nil
	}
	raw, present := cfg["security"]
	if !present || raw == nil {
		return 0, false, nil
	}
	selection, ok := raw.(map[string]any)
	if !ok {
		return 0, false, fmt.Errorf("configuration.security must be an object containing index")
	}
	index, ok := configIndex(selection["index"])
	if !ok || index < 0 {
		return 0, false, fmt.Errorf("configuration.security.index must be a non-negative integer")
	}
	return index, true, nil
}

func securityPlansForSelection(plans []securityPlan, bindCtx map[string]any) ([]securityPlan, error) {
	index, selected, err := configuredSecurityAlternative(bindCtx)
	if err != nil || !selected {
		return plans, err
	}
	result := make([]securityPlan, 0, len(plans))
	for _, plan := range plans {
	...
	if len(result) == 0 {
		return nil, fmt.Errorf("configuration.security.index %d selects no usable complete security alternative", index)
	}
	return result, nil
}

func securitySelectionConfigurationError(doc *openapi3.T, op *openapi3.Operation, baseURL string, params openapi3.Parameters, bindCtx map[string]any) error {
	_, err := securityPlansForSelection(viableSecurityPlans(doc, op, baseURL, params), bindCtx)
	return err
}
...
func securityConfigurationError(doc *openapi3.T, op *openapi3.Operation) error {
	...
	if len(securityPlans(doc, op, "")) > 0 {
		// Undefined/malformed alternatives are confined; a complete sibling
		// prevents them from weakening or poisoning the effective OR set.
		return nil
	}
...
func securityPlans(doc *openapi3.T, op *openapi3.Operation, baseURL string) []securityPlan {
	...
	for alternativeIndex, secReq := range *requirements {
		if traceExcludesSecurityAlternative(op, alternativeIndex) {
			continue
		}
		if len(secReq) == 0 {
			plans = append(plans, securityPlan{alternativeIndex: alternativeIndex})
			continue
		}
		expanded := []securityPlan{{alternativeIndex: alternativeIndex}}
		expressible := true
		...
		for _, schemeName := range names {
			...
			if malformedSecuritySchemeForEdition(ref.Value, doc.OpenAPI) {
				expressible = false
				break
			}
...
func selectCredentialPlacements(doc *openapi3.T, op *openapi3.Operation, bindCtx map[string]any, baseURL string, params openapi3.Parameters, populated map[string]map[string]bool) ([]credentialPlacement, []namedSecurityScheme, []credentialPlacement, error) {
	...
	plans, err := securityPlansForSelection(viableSecurityPlans(doc, op, baseURL, params), bindCtx)
	if err != nil {
		return nil, nil, nil, err
	}
	complete := []securityPlan{}
	for _, plan := range plans {
		if len(plan.context.Requirements) == 0 {
	...
	if _, explicitlySelected, _ := configuredSecurityAlternative(bindCtx); !explicitlySelected {
		distinct := map[int]bool{}
		for _, plan := range complete {
			distinct[plan.alternativeIndex] = true
		}
		if len(distinct) > 1 {
			return nil, nil, nil, fmt.Errorf("multiple complete security alternatives require configuration.security selection")
		}
	}
	for _, plan := range complete {
		if len(plan.context.Requirements) == 0 {
...
func validateCredentialWireValues(plan securityPlan, bindCtx map[string]any, placements []credentialPlacement) error {
	...
	for _, placement := range placements {
		if placement.channel == "header" && !validSerializedHeaderFieldValue(placement.value) {
			return fmt.Errorf("header credential %q is not a complete RFC 9110 field-value", placement.name)
		}
		if placement.channel == "header" && strings.EqualFold(placement.name, "Cookie") && !validRFC6265CookieString(placement.value) {
			return fmt.Errorf("header credential %q is not an RFC 6265 cookie-string", placement.name)
		}
...
func validRFC6265CookieString(value string) bool {
	...
	for _, unit := range units {
		...
		if separator <= 0 || !httpToken(unit[:separator]) || !validRFC6265CookieValue(unit[separator+1:]) {
			return false
		}
	}
	return true
...
func checkCredentialCollisions(placements []credentialPlacement, params openapi3.Parameters, populated map[string]map[string]bool) error {
	...
	requiredWholeQuery := false
	requiredStructuredCookie := false
	for _, ref := range params {
		if ref == nil || ref.Value == nil {
			continue
		}
		if ref.Value.In == ParameterInQueryString && ref.Value.Required {
			requiredWholeQuery = true
		}
		if ref.Value.In == openapi3.ParameterInCookie && ref.Value.Required {
			requiredStructuredCookie = true
		}
		if !ref.Value.Required || !parameterDestinationStaticallyFixed(ref.Value) {
			continue
		}
		name := ref.Value.Name
	...
	hasStructuredCookieOwner := requiredStructuredCookie || populated != nil && len(populated[openapi3.ParameterInCookie]) > 0
	for _, placement := range placements {
	...
	for _, pl := range placements {
		name := pl.name
		if pl.channel == "query" && (requiredWholeQuery || populated != nil && len(populated[ParameterInQueryString]) > 0) {
			return fmt.Errorf("query credential %q collides with the effective whole-querystring parameter", pl.name)
		}
		if pl.channel == "header" {
			name = http.CanonicalHeaderKey(name)
...
func parameterDestinationStaticallyFixed(parameter *openapi3.Parameter) bool {
	if parameter == nil || parameter.In == ParameterInQueryString {
		return false
	}
	if len(parameter.Content) > 0 || parameter.In == openapi3.ParameterInHeader || parameter.In == openapi3.ParameterInPath {
		return true
	}
	...
	explode := style == openapi3.SerializationForm || style == "cookie"
	if parameter.Explode != nil {
		explode = *parameter.Explode
	}
	if style == openapi3.SerializationDeepObject {
		return false
	}
	if (style == openapi3.SerializationForm || style == "cookie") && explode {
		return false
	}
	return true
}
