...
//
// Whole-source refusal is §3.2's three-part text (`openbindings.openapi-3.1@1`
// and its siblings): part
...
func computeAcceptanceFloor(root map[string]any) *acceptanceFloor {
	...
	if pathsIsObj {
		for _, pathKey := range floorKeys(paths) {
			...
			for _, method := range httpMethods {
				...
				ref := "#/paths/" + floorEsc(pathKey) + "/" + method
				...
				for _, holder := range []struct {
					...
					for i, p := range list {
						...
						_, hasSchema := pm["schema"]
						_, hasContent := pm["content"]
						if hasSchema && hasContent {
							addDefect(defect(floorD17, holder.ptr+"/"+strconv.Itoa(i)))
						}
					}
				}
	...
	addressableTargets := 0

	for _, row := range rawOps {
		// The HTTP-method member is the structurally declared target slot. A
		// defect at that target or below changes usability/coverage, not
		// addressability; selectors must still resolve it to a pre-dispatch
		// refusal. Only inventory/reference defects that prevent a slot from
		// being formed contribute to the source-refusal aggregation below.
		addressableTargets++
		...
		for _, code := range codes {
			...
			// Response defects are confined to their smallest projection. The
			// admitted key keeps lookup precedence, valid content/header siblings
			// remain governing, and runtime outcome is decided from the actual
			// status/body. Documentary fields never change invocation behavior.
			if respDefective {
				addProjection(op.Ref, respDefect)
			}
			...
			if len(deadAlternativeDefects) > 0 {
				// Some response media alternatives are invalid but the response
				// key and operation remain addressable at the operation projection.
	...
	// §3 part 2, the single derived rule: refuse only when inventory or
	// reference defects prevent every declared slot from becoming addressable.
	// A target-confined invalidity or exclusion never contributes to this
	// aggregation, even when every addressable target is unusable.
	if addressableTargets == 0 && anyDestroyedOrInvalid && f.ExternalPathItemMembers == 0 {
