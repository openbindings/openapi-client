...
func validateRevision3MultipartMedia(doc *openapi3.T, media *openapi3.MediaType) error {
	...
	for name := range props {
		...
		if _, err := resolvedEncodingHeaders(enc, is30, name); err != nil {
...
func encodeTextString(value string, parsed parsedMediaType) ([]byte, error) {
	return encodeTextStringWith(value, parsed, nil)
}

func encodeTextStringWith(value string, parsed parsedMediaType, encoders map[string]CharacterEncoder) ([]byte, error) {
	charset := "utf-8"
...
func requestMediaUnconfigured(bindCtx map[string]any) bool {
	raw, configured := contextConfiguration(bindCtx)["requestMedia"]
...
func buildRequestBody(doc *openapi3.T, plan *bodyPlan, routed *routedInput) (io.Reader, string, error) {
	...
	case familyJSON:
		if plan.synthetic || plan.wholeObject {
			...
			b, err := marshalRequestJSON(routed.bodyValue)
			if err != nil {
				return nil, "", err
			}
		...
		if len(routed.bodyFields) == 0 {
			if plan.required || routed.bodyPresent {
				return strings.NewReader("{}"), plan.mediaType, nil
			}
			return nil, "", nil
		}
		b, err := marshalRequestJSON(routed.bodyFields)
		if err != nil {
			return nil, "", err
		}
		return bytes.NewReader(b), plan.mediaType, nil
	...
	case familyText:
		...
		if plan.oas32 {
			if plan.mediaRange {
				types, constrained, ambiguous := openAPI32ResolvedTypes(mediaSchema(plan.media), map[*openapi3.Schema]bool{})
				nonNull := 0
				...
				if ambiguous || !constrained || nonNull != 1 {
					return nil, "", fmt.Errorf("request media %s does not determine exactly one non-JSON serialization type", plan.mediaType)
				}
			}
			...
			encoded, err := encodeTextStringWith(text, parsed, plan.requestCharacterEncodings)
			if err != nil {
				return nil, "", fmt.Errorf("request media %s: %w", plan.mediaType, err)
			}
		...
		if hasMediaFidelity(plan.bindingSpec) {
			...
			encoded, err := encodeTextStringWith(s, parsed, plan.requestCharacterEncodings)
			if err != nil {
				return nil, "", fmt.Errorf("request media %s: %w", plan.mediaType, err)
			}
			return bytes.NewReader(encoded), plan.mediaType, nil
	...
	for _, name := range names {
		...
		if hasMediaFidelity(bindingSpec) {
			...
			if partNullable && value == nil {
				if resolveDeclaration(schema, is30).requiresProperty(name) {
					return nil, "", fmt.Errorf("required multipart part %q cannot represent null by elision", name)
				}
...
func writeRevision3MultipartPart(writer *multipart.Writer, name string, value any, schema *openapi3.Schema, enc *openapi3.Encoding, is30 bool) error {
	...
	case revision3PropertyJSON:
		body, err = marshalRequestJSON(value)
	...
	h := make(textproto.MIMEHeader)
	encodingHeaders, err := resolvedEncodingHeaders(enc, is30, name)
	if err != nil {
		return fmt.Errorf("multipart part %q: %w", name, err)
	}
	for headerName, headerValue := range encodingHeaders {
...
// resolvedEncodingHeaders compiles the binding-governing subset of an
// Encoding Object's Header map. HTTP case variants denote one field. Exact
// finite raw-string domains compose conjunctively; one provable singleton is
// emitted once, while a required group with no singleton and every provably
// empty intersection makes the multipart alternative unusable.
func resolvedEncodingHeaders(encoding *openapi3.Encoding, is30 bool, partName string) (map[string]string, error) {
	type member struct {
		name     string
		required bool
	...
	for name, ref := range encoding.Headers {
		if strings.EqualFold(name, "Content-Type") || ref == nil || ref.Value == nil {
			continue
		}
		if !httpToken(name) {
			return nil, fmt.Errorf("Encoding header name %q is not an HTTP token", name)
		}
		domain, known := exactHeaderStringDomain(ref.Value.Schema, is30)
		groups[strings.ToLower(name)] = append(groups[strings.ToLower(name)], member{
			name: name, required: ref.Value.Required, domain: domain, known: known,
		})
	}
	...
	for _, members := range groups {
		...
		for _, candidate := range members {
			required = required || candidate.required
			if candidate.known && len(candidate.domain) == 0 {
				return nil, fmt.Errorf("Encoding header group %q has an empty exact string domain", candidate.name)
			}
			if candidate.known && len(candidate.domain) == 1 {
				for value := range candidate.domain {
					if hasFixed && fixed != value {
						return nil, fmt.Errorf("case-equivalent Encoding headers fix conflicting values %q and %q", fixed, value)
					}
					fixed, hasFixed = value, true
				}
			}
		}
		if hasFixed {
			for _, candidate := range members {
				if candidate.known && !candidate.domain[fixed] {
			...
			if strings.EqualFold(members[0].name, "Content-Disposition") {
				if hasHTTPParameterName(fixed, "filename*") {
					return nil, fmt.Errorf("Encoding Content-Disposition uses forbidden filename*")
				}
				disposition, parameters, err := mime.ParseMediaType(fixed)
				if err != nil || !strings.EqualFold(disposition, "form-data") || parameters["name"] != partName {
					return nil, fmt.Errorf("Encoding Content-Disposition does not preserve form-data name %q", partName)
				}
			}
			result[members[0].name] = fixed
			continue
		}
		if required {
...
func hasHTTPParameterName(value, wanted string) bool {
	parts, err := splitHTTPMediaType(value)
	if err != nil {
		return false
	}
	for _, part := range parts[1:] {
		if separator := strings.IndexByte(part, '='); separator > 0 && strings.EqualFold(strings.TrimSpace(part[:separator]), wanted) {
			return true
		}
	}
	return false
...
func exactHeaderStringDomain(ref *openapi3.SchemaRef, is30 bool) (map[string]bool, bool) {
	if ref == nil || ref.Value == nil {
		return nil, false
	}
	declaration := resolveDeclaration(ref.Value, is30)
	...
	for _, conjunct := range declaration.conjuncts {
		...
		if !is30 && conjunct.Const != nil {
			candidates = map[string]bool{}
			if value, ok := conjunct.Const.(string); ok {
				candidates[value] = true
			}
		} else if len(conjunct.Enum) > 0 {
			candidates = map[string]bool{}
			for _, raw := range conjunct.Enum {
	...
	for _, name := range names {
		...
		if hasMediaFidelity(bindingSpec) && !encodingUsesSerialization(enc) {
			...
			if propertyNullable && fields[name] == nil {
				if resolveDeclaration(schema, is30).requiresProperty(name) {
					return "", fmt.Errorf("required form field %q cannot represent null by elision", name)
				}
...
func revision3PropertyBytes(name string, value any, schema *openapi3.Schema, contentType parsedMediaType, is30 bool) ([]byte, error) {
	...
	switch mode {
	case revision3PropertyJSON:
		return marshalRequestJSON(value)
	case revision3PropertyRawOctets:
		text, ok := value.(string)
		if !ok {
...
// successMediaTypesFor retains its historical name, but the generated Accept
// set is the union of every admitted response-content alternative. Accept is
// request negotiation, not a claim that a failure-only Response can govern a
// successful status; actual response lookup remains exact/range/default.
func successMediaTypesFor(op *openapi3.Operation, bindingSpec string) []string {
	...
	responses := op.Responses.Map()
	for responseKey, ref := range responses {
		if ref == nil || ref.Value == nil {
			continue
		}
		if responseKeyOnlyForbidsContent(responseKey) {
			continue
		}
		...
		for mt := range ref.Value.Content {
			...
			if hasMediaFidelity(bindingSpec) {
				if _, reserved := parsed.params["q"]; reserved {
					continue
				}
			}
...
func responseKeyOnlyForbidsContent(key string) bool {
	if key == "1XX" {
		return true
	}
	if len(key) != 3 || key[0] < '0' || key[0] > '9' || key[1] < '0' || key[1] > '9' || key[2] < '0' || key[2] > '9' {
		return false
	}
	status := int(key[0]-'0')*100 + int(key[1]-'0')*10 + int(key[2]-'0')
