...
func swagger20ResponsesFor(graph *swagger20ReferenceGraph, operation swagger20Operation) (swagger20ResponseSet, error) {
	...
	for key, value := range responses.value {
		...
		if key != "default" && !swagger20ExactStatusKey(key) {
			// An inadmissible key cannot govern an HTTP response. Confining that
			// entry as invalid preserves every admitted sibling and lookup never
			// considers it.
			continue
		}
		count++
...
func (s swagger20ResponseSet) governing(graph *swagger20ReferenceGraph, status int) (*swagger20ResolvedResponse, string, error) {
	...
	if err != nil {
		return &swagger20ResolvedResponse{raw: swagger20Object{}, resource: s.resource, invalid: true}, key, nil
	}
...
func resolveSwagger20Response(graph *swagger20ReferenceGraph, value any, resource *swagger20Resource, active map[string]bool) (swagger20ResolvedResponse, error) {
	...
	if !ok {
		return swagger20ResolvedResponse{raw: swagger20Object{}, resource: resource, invalid: true}, nil
	}
...
func handleSwagger20Response(request *http.Request, response *http.Response, prepared *Swagger20PreparedOperation, responses swagger20ResponseSet, execution *Execution) {
	...
	if response.StatusCode == http.StatusSwitchingProtocols {
		swagger20FailResponse(execution, fmt.Errorf("101 Switching Protocols cannot complete this unary binding"))
		return
	}
	codings, err := validateSwagger20ResponseContentCodings(response, governing)
	if err != nil {
		swagger20FailResponse(execution, err)
		return
	}
	noContentStatus := response.StatusCode >= 100 && response.StatusCode < 200 || response.StatusCode == 204 || response.StatusCode == 205 || response.StatusCode == 304
	if request.Method == http.MethodHead || noContentStatus {
		if request.Method != http.MethodHead && len(body) > 0 {
			swagger20FailResponse(execution, fmt.Errorf("HTTP response carries content where none is permitted"))
			return
		}
		body = nil
	} else if err := applySwagger20ResponseContentCodings(codings, &body, prepared.options.ResponseContentCodings); err != nil {
		swagger20FailResponse(execution, err)
		return
	}
	...
	value, err := decodeSwagger20ResponseValue(selection, body, prepared.options.ResponseCharacterEncodings)
	if err != nil {
		swagger20FailResponse(execution, err)
		return
	}
...
func validateSwagger20ResponseContentCodings(response *http.Response, governing *swagger20ResolvedResponse) ([]string, error) {
	raw, err := singletonResponseHeader(response.Header, "Content-Encoding")
	if err != nil || raw == "" {
		return nil, err
	}
	if governing != nil {
		headers, err := swagger20ResponseHeaders(governing.raw, "Content-Encoding")
		if err != nil {
			return nil, err
		}
		for _, header := range headers {
			if err := swagger20HeaderObjectAdmits(header, raw); err != nil {
				return nil, fmt.Errorf("actual response Content-Encoding is not admitted by its governing Header Object: %w", err)
			}
		}
	}
...
func applySwagger20ResponseContentCodings(tokens []string, body *[]byte, codecs map[string]ContentDecoder) error {
	var err error
...
func swagger20ResponseHeader(response swagger20Object, name string) (swagger20Object, error) {
	headers, err := swagger20ResponseHeaders(response, name)
...
func swagger20ResponseHeaders(response swagger20Object, name string) ([]swagger20Object, error) {
	...
	found := []swagger20Object{}
	names := make([]string, 0, len(headers.value))
	for declared := range headers.value {
	...
	for _, declared := range names {
		...
		value := headers.value[declared]
		object, ok := value.(map[string]any)
		if !ok {
			continue
		}
		found = append(found, swagger20Object(object))
	}
	return found, nil
}
...
func swagger20HeaderObjectAdmits(header swagger20Object, value string) error {
	...
	enumeration := header.array("enum")
	if !enumeration.present || !enumeration.valid {
		return nil
	}
	domain := map[string]bool{}
	for _, member := range enumeration.value {
		if text, ok := member.(string); ok {
			domain[text] = true
		}
	}
	if len(domain) > 0 && !domain[value] {
		return fmt.Errorf("value is outside the binding-understood finite string domain")
	}
	return nil
}

func decodeSwagger20ResponseValue(selection *swagger20ResponseMediaSelection, body []byte, decoders ...map[string]CharacterDecoder) (any, error) {
	...
	case swagger20LaneText:
		var configured map[string]CharacterDecoder
		if len(decoders) > 0 {
			configured = decoders[0]
		}
		return decodeTextLaneWith(selection.media.canonical, body, profileFullCoordinate, configured)
