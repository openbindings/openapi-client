...
`)}, ArtifactLoadOptions{})
	...
	if operations := artifact.Operations(); len(operations) != 1 || operations[0].Ref != "#/paths/~1good/get" {
		t.Fatalf("confined operation inventory = %#v", operations)
	}
	if operations := enumerateOperationsWithFloor(artifact, nil); len(operations) != 2 || operations[0].unusable == nil {
		t.Fatalf("addressable internal inventory = %#v", operations)
	}
}
