...
func loadArtifact(ctx context.Context, client *http.Client, source Source, allowExternalRefs bool) (*Artifact, *acceptanceFloor, error) {
	...
	attempt := func(entryOverride []byte) (*openapi3.T, error) {
		...
		loader.ReadFromURIFunc = func(loader *openapi3.Loader, resource *url.URL) ([]byte, error) {
			...
			loadedResourcesMu.Lock()
			for _, identity := range []*url.URL{resource, retrieval} {
				if key := artifactResourceKey(identity); key != "" {
					loadedResources[key] = append([]byte(nil), data...)
				}
			}
			loadedResourcesMu.Unlock()
			if laneOverlay != nil {
			...
			data, err = normalizer.markReferringSecuritySchemes(data, resource, retrieval)
			if err != nil {
				return nil, err
			}
	...
	artifact := &Artifact{
		...
		referringSecurityByPath: referringSecurityScopes(entryBytes, source.Location, loadedResources),
