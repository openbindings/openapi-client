...
`)}, ClientOptions{HTTPClient: &http.Client{Transport: transport}})
	...
	if _, err := client.Call(context.Background(), PathOperation("/range-text", POST), Input{
		BodyPresent: true, Body: true, MediaType: "text/plain",
	}); err == nil || !strings.Contains(err.Error(), "does not determine exactly one non-JSON serialization type") {
		t.Fatalf("range text ambiguity error = %v", err)
	}
	if len(transport.requests) != before {
		t.Fatal("ambiguous range-selected text request dispatched")
	}
}
