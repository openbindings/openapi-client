...
func selectSwagger20Security(document *Swagger20Document, operation swagger20Operation, parameters *swagger20ParameterSet, selection *int, credentials Swagger20SecurityCredentials) ([]swagger20CredentialPlacement, error) {
	...
	for _, name := range names {
		...
		case "apiKey":
			...
			if destination.value == "header" && strings.EqualFold(wireName.value, "Cookie") && !swagger20CookieString(value) {
				return nil, fmt.Errorf("Swagger 2.0 apiKey credential %q is not a complete RFC 6265 Cookie field value", name)
			}
...
func swagger20CredentialCollidesWithParameter(placement swagger20CredentialPlacement, parameters *swagger20ParameterSet) bool {
	...
	if placement.query {
		parameter := parameters.byWire[Swagger20ParameterQuery][placement.name]
		return parameter != nil && parameter.required
	}
	for name, parameter := range parameters.byWire[Swagger20ParameterHeader] {
		if strings.EqualFold(name, placement.name) && parameter.required {
			return true
		}
	}
	return false
...
func swagger20CredentialCollidesWithRoutedInput(placement swagger20CredentialPlacement, routed swagger20RoutedInput) bool {
	if placement.query {
		encoded := swagger20PercentEncode(placement.name)
		for _, contribution := range routed.query {
			if contribution.name == encoded {
				return true
			}
		}
		return false
	}
	for _, contribution := range routed.headers {
		if strings.EqualFold(contribution.name, placement.name) {
			return true
		}
	}
	return false
...
func swagger20ProcessorOwnedHeader(name string) bool {
	...
	case "host", "content-length", "content-type", "content-encoding", "accept", "accept-encoding",
