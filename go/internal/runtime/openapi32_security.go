...
func malformedSecuritySchemeForEdition(scheme *openapi3.SecurityScheme, edition string) bool {
	if malformedOpenAPI32SecurityScheme(scheme) {
		return true
	}
	// A header API key owns an HTTP field-name, so the binding's accepted
	// security surface is narrower than the OAS string-valued `name` field.
	// Confine this unusable destination to its complete security alternative;
	// never let net/http sanitize or reject it after dispatch has begun.
	if scheme.Type == "apiKey" && scheme.In == "header" && !httpToken(scheme.Name) {
		return true
	}
	// mutualTLS entered the closed Security Scheme type set in OAS 3.1.
	return strings.HasPrefix(edition, "3.0.") && scheme.Type == "mutualTLS"
}
