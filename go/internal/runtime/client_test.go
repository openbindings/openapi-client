...
func testOpenAPI32SSEDocument(serverURL string) []byte {
	return []byte(fmt.Sprintf(`{
  "openapi":"3.2.0",
  "info":{"title":"test","version":"1"},
  "servers":[{"url":%q}],
  "paths":{"/events":{"get":{"operationId":"events","responses":{"200":{"content":{"text/event-stream":{"itemSchema":{"type":"object"}}}}}}}}
}`, serverURL))
}
...
func TestClientKeepsRedirectsObservableByDefaultAndFollowsOnlyPreservingRedirects(t *testing.T) {
  ...
  "/preserve":{"post":{"operationId":"preserve","requestBody":{"required":true,"content":{"application/json":{"schema":{"type":"object"}}}},"responses":{"204":{"description":"done"}}}}
	...
	rewrite, callErr := following.Call(context.Background(), OperationID("rewrite"), Input{
		Body: map[string]any{"value": "rewrite"}, BodyPresent: true,
	})
	if callErr != nil || rewrite.OK || rewrite.Response == nil || rewrite.Response.StatusCode != http.StatusSeeOther {
		t.Fatalf("method-rewriting redirect: result=%#v err=%v", rewrite, callErr)
	}
	preserve, callErr := following.Call(context.Background(), OperationID("preserve"), Input{
		Body: map[string]any{"value": "preserve"}, BodyPresent: true,
	})
	if callErr != nil || !preserve.OK {
		t.Fatalf("method-preserving redirect: result=%#v err=%v", preserve, callErr)
	}
	if _, present := observed["/rewrite-final"]; present {
		t.Fatal("configured following rewrote the artifact-bound POST to GET")
	}
...
func TestClientSSEPreservesOrderingFramingAndCompletion(t *testing.T) {
	...
	defer server.Close()
	document := testOpenAPI32SSEDocument(server.URL)
	client, err := Load(context.Background(), Source{Content: document}, ClientOptions{})
	if err != nil {
		t.Fatal(err)
	}
	...
	wantData := []any{
		map[string]any{"data": "first", "event": "update", "id": "7", "retry": 50},
		map[string]any{"data": "second"},
	}
	if len(events) != 2 || !reflect.DeepEqual([]any{events[0].Data, events[1].Data}, wantData) {
...
func TestStreamPreservesPartialOutputBeforeCancellation(t *testing.T) {
	...
	if err != nil || !open || !reflect.DeepEqual(event.Data, map[string]any{"data": "partial"}) {
