...
func (d *Swagger20Document) rebind(ctx context.Context, client *http.Client, allowExternalRefs bool) *Swagger20Document {
	...
	// The reference walker switches on the JSON representation types. Keep the
	// graph root as the underlying map rather than the named convenience view,
	// or a rebound in-memory document is mistaken for a scalar at its first
	// JSON Pointer token.
	entry := graph.rememberResolvedResource(d.entry.requested, d.entry.retrieval, map[string]any(d.root))
...
func (o swagger20Operation) operationInfo(ref string) OperationInfo {
	...
	info := OperationInfo{
		Ref: ref, Path: o.path, Method: Method(o.method), WireMethod: swagger20HTTPMethod(o.method), Additional: false,
