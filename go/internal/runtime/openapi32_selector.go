...
func parseOperationReference(ref string, edition Edition) (OperationReference, error) {
	...
	if edition.IsOpenAPI32() && len(parts) == 3 && parts[1] == "additionalOperations" {
		...
		for _, fixed := range openAPI32FixedMethods {
			// The key denotes the method token it spells, and OAS forbids only
			// an entry for a method a fixed field already defines. RFC 9110
			// §9.1 makes the method token case-sensitive and OAS 3.2.0 §3.2
			// routes this comparison to HTTP's own rule, so the collision is
			// byte-equality against the token this fixed field sends —
			// strings.ToUpper(fixed), which is WireMethod's fixed-field rule.
...
func requestTargetForEdition(target *OperationTarget, edition Edition) *OperationTarget {
	if target == nil || target.Operation == nil || !methodIgnoresRequestBody(edition, target.Additional, target.Method) {
		return target
	}
...
func methodIgnoresRequestBody(edition Edition, additional bool, method string) bool {
	method = strings.ToLower(method)
	if edition == EditionOpenAPI300 || edition == EditionOpenAPI301 || edition == EditionOpenAPI302 || edition == EditionOpenAPI303 || edition == EditionOpenAPI304 {
		switch method {
		case "get", "head", "delete", "options", "trace":
			return true
		}
	}
	return !additional && method == "trace"
}
