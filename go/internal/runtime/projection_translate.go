...
func translateSchemaDialect(schema map[string]any, openapiVersion string) map[string]any {
	...
	w := &schemaWalker{legacy: projectionIsOpenAPI30(openapiVersion)}
	out, _ := w.node(schema, "").(map[string]any)
	return out
}

func projectionIsOpenAPI30(version string) bool {
...
func (w *schemaWalker) object(in map[string]any, path string) map[string]any {
	...
	if nullable, ok := in["nullable"].(bool); w.legacy && ok && nullable {
		...
		case string:
			out["type"] = []any{t, "null"}
		case []any:
			if projectionContainsString(t, "null") {
				cp := make([]any, len(t))
				copy(cp, t)
				out["type"] = cp
			} else {
...
func projectionContainsString(arr []any, s string) bool {
