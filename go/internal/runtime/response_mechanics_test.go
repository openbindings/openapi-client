...
func TestHEADResponseIsEmptyAndRequiredHeadersAreGoverned(t *testing.T) {
	...
	forbidden := &http.Response{StatusCode: 200, Header: http.Header{"X-Required": {"yes"}}, Body: io.NopCloser(strings.NewReader("forbidden")), Request: request}
	if _, err := applyResponseMechanics(request, forbidden, document, operation, profileFullCoordinate, nil, true); err == nil || !strings.Contains(err.Error(), "forbidden content") {
		t.Fatalf("HEAD forbidden-content error = %v", err)
	}
	response := &http.Response{StatusCode: 200, Header: http.Header{"X-Required": {"yes"}}, Body: io.NopCloser(strings.NewReader("")), Request: request}
