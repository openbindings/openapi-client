...
func (s *ServerSet) Resolve(selection *ServerSelection) (string, error) {
	if s != nil && selection != nil && selection.URL != "" {
		if !denotesTargetBase(selection.URL) {
			return "", fmt.Errorf("server URL %q is not an absolute target URL", selection.URL)
		}
		return absolutizeServerURL(selection.URL, s.sourceLocation)
	}
	if s == nil || len(s.servers) == 0 {
		return "", &ServerResolutionRequiredError{
			Path:        "/url",
			Description: "the effective server list has no usable alternative; supply a complete base URL",
		}
	}
...
func resolveServer(doc *openapi3.T, pathItem *openapi3.PathItem, op *openapi3.Operation, bindCtx map[string]any, sourceLocation string) (string, error) {
	...
	if meta := contextMetadata(bindCtx); meta != nil {
		if base, ok := meta["baseURL"].(string); ok && base != "" {
			resolved, err := set.Resolve(&ServerSelection{URL: base})
			return resolved, serverConfigRequired(err)
		}
	}
	resolved, err := set.Resolve(nil)
	if err == nil && !httpServerBase(resolved) {
		err = &ServerResolutionRequiredError{
			Path:        "/url",
			Description: fmt.Sprintf("artifact-derived server URL %q cannot carry this HTTP invocation; supply a complete http or https base URL", resolved),
		}
	}
	return resolved, serverConfigRequired(err)
}

// httpServerBase is the declaration-derived dispatchability boundary. A
// non-HTTP Server Object remains a represented, recoverable target, but it
// creates an actual configuration.server requirement instead of becoming a
// late ordinary refusal after the operation has already been selected.
func httpServerBase(value string) bool {
	parsed, err := url.Parse(value)
	if err != nil {
		return false
	}
	scheme := strings.ToLower(parsed.Scheme)
	return (scheme == "http" || scheme == "https") && parsed.Hostname() != "" && parsed.User == nil
}
...
func serverSelectionFromConfig(raw any, servers openapi3.Servers) (*ServerSelection, error) {
	...
	case map[string]any:
		if base, ok := v["baseUrl"].(string); ok && base != "" {
			return &ServerSelection{URL: base}, nil
		}
...
func substituteServerVariablesFor(srv *openapi3.Server, supplied map[string]string, edition string) (string, error) {
	...
	for _, name := range names {
		...
		if extensionBool(v.Extensions, serverVariableInvalidMarker) {
			return "", fmt.Errorf("server %q variable %q has an invalid fixed-field value", srv.URL, name)
		}
		expression := "{" + name + "}"
	...
	for {
		opening := strings.IndexByte(u, '{')
		if opening < 0 {
			break
		}
		closingOffset := strings.IndexByte(u[opening+1:], '}')
		if closingOffset < 0 {
			return "", fmt.Errorf("server URL %q contains a malformed template expression", srv.URL)
		}
		closing := opening + 1 + closingOffset
		name := u[opening+1 : closing]
		...
		if !present {
			return "", &ServerResolutionRequiredError{
				...
				Description: fmt.Sprintf("server %q: URL expression %q has no matching declaration-derived default or supplied value", srv.URL, name),
			}
		}
		used[name] = true
		u = u[:opening] + value + u[closing+1:]
	}
...
// resolved Server URL, removing exactly the redundant slash at that one
// composition seam. No other path normalization is performed.
func AssembleRequestURL(serverBase, operationPath, rawQuery string) (*url.URL, error) {
	if strings.HasSuffix(serverBase, "/") && strings.HasPrefix(operationPath, "/") {
		serverBase = serverBase[:len(serverBase)-1]
	}
...
func ValidateRequestURL(completed *url.URL) error {
	...
	if parsed.Hostname() == "" {
		return fmt.Errorf("completed OpenAPI http(s) target has an empty host")
	}
	if parsed.User != nil {
		return fmt.Errorf("completed OpenAPI http(s) target contains forbidden userinfo")
	}
...
func eligibleServers(servers openapi3.Servers, edition, sourceLocation string) (openapi3.Servers, error) {
	...
	// Every defect above belongs to a Server alternative. The target remains
	// recoverable through the complete configured URL boundary even when no
	// declaration-derived alternative survives.
	_ = first
	return nil, nil
}
