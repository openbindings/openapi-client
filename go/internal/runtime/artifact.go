...
type Artifact struct {
	...
	referringSecurityByPath map[string]openapi3.SecuritySchemes
	sourceRefusal           string
	sourceExclusion         string
}

func openAPI32ArtifactDisposition(document *openapi3.T) (refusal, exclusion string) {
	if document == nil || !(strings.HasPrefix(document.OpenAPI, "3.1.") || document.OpenAPI == string(EditionOpenAPI320)) {
		return "", ""
	}
	if document.OpenAPI == string(EditionOpenAPI320) && document.Components == nil && document.Paths == nil && document.Webhooks == nil {
