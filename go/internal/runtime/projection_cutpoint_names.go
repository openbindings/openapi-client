...
// internalizeProjectionRefs closes the already-loaded reference graph exactly
// once, while the artifact is still being constructed. Projection therefore
// never fetches, reparses, or mutates a Client after publication, while its
// operation-local schemas cannot retain external or components-relative refs.
func internalizeProjectionRefs(ctx context.Context, doc *openapi3.T) map[string]refIdentity {
