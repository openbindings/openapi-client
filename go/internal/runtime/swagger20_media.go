...
func swagger20GeneratedAccept(document *Swagger20Document, operation swagger20Operation, parameters *swagger20ParameterSet, responses swagger20ResponseSet) (string, error) {
	for _, parameter := range parameters.nonBody {
		if parameter.in == Swagger20ParameterHeader && strings.EqualFold(parameter.name, "Accept") {
			return "", nil
		}
	}
	...
	produces, err := effectiveSwagger20MediaSet(document, operation, "produces")
	if err != nil {
		return "", err
	}
	declarations := []swagger20SchemaDeclaration{}
	for key, value := range responses.values {
		if !swagger20ResponseKeyCanGovernContentSuccess(key, responses.values) {
			continue
		}
		response, resolveErr := resolveSwagger20Response(document.graph, value, responses.resource, map[string]bool{})
		if resolveErr != nil || response.invalid {
			continue
		}
		schema, present := response.raw.member("schema")
		if !present {
			continue
		}
		declaration, declarationErr := resolveSwagger20SchemaDeclaration(document.graph, schema, response.resource, true)
		if declarationErr == nil {
			declarations = append(declarations, declaration)
		}
	}
	values := map[string]string{}
	for _, entry := range produces.entries {
		if entry.parseErr != nil || entry.colliding {
			continue
		}
		if _, reservedWeight := entry.parsed.params["q"]; reservedWeight {
			continue
		}
		for _, declaration := range declarations {
			if swagger20ResponseRangeHasUsableLane(entry.parsed, declaration) {
				values[entry.parsed.semanticIdentity] = entry.parsed.canonical
				break
			}
		}
	}
...
func swagger20ResponseKeyCanGovernContentSuccess(key string, responses map[string]any) bool {
	if key == "default" {
		for status := 200; status <= 299; status++ {
			if status == http.StatusNoContent || status == http.StatusResetContent {
				continue
			}
			if _, exact := responses[strconv.Itoa(status)]; !exact {
				return true
			}
		}
		return false
	}
	if !swagger20ExactStatusKey(key) || key[0] != '2' {
		return false
	}
...
func swagger20ResponseRangeHasUsableLane(declaration parsedMediaType, schema swagger20SchemaDeclaration) bool {
	if declaration.rangeSpecificity == 2 {
		_, err := swagger20ResponseLane(declaration, schema)
		return err == nil
	}
	for _, candidate := range []string{"application/json", "text/plain", "application/octet-stream", "image/png"} {
		...
		if requestMediaDeclarationMatches(declaration, parsed) {
			if _, err := swagger20ResponseLane(parsed, schema); err == nil {
				return true
			}
		}
	}
...
func encodeSwagger20RequestPayload(selection *swagger20RequestMediaSelection, model swagger20PayloadModel, routed swagger20RoutedInput, options Swagger20PrepareOptions) ([]byte, string, error) {
	...
	case swagger20LaneJSON:
		body, err := marshalRequestJSON(routed.body)
		if err != nil {
	...
	case swagger20LaneText:
		...
		body, err := encodeTextStringWith(text, selection.media, options.RequestCharacterEncodings)
		if err != nil {
			return nil, "", err
		}
		return body, selection.media.canonical, nil
