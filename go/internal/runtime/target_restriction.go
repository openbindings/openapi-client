...
// the ladder ALREADY marks invalid over a climbing defect that the typed loader
// cannot represent: a Response Object defect (D7, D9 or D16), or a Parameter
// Object declaring both mutually-exclusive representation fields (D17). Under
// that condition the pass decides no exclusion of its
...
// restrictedTypedLoadDefectClasses are the climbing defects for which the raw
// acceptance instrument already owns the target-level verdict while the typed
// dependency cannot construct an in-memory node. The restriction pass changes
// only failure scope, never acceptance semantics.
var restrictedTypedLoadDefectClasses = map[string]bool{
	floorD7: true, floorD9: true, floorD16: true, floorD17: true,
}

// restrictedTypedLoadDefectiveTargets is the set of operations the ladder
// marks invalid over a defect admitted by the per-target restriction pass.
func restrictedTypedLoadDefectiveTargets(floor *acceptanceFloor) map[string]bool {
	out := map[string]bool{}
	if floor == nil {
		return out
	}
	for ref, op := range floor.Ops {
		...
		for _, defect := range op.Defects {
			if restrictedTypedLoadDefectClasses[defect.Class] {
				out[ref] = true
				break
			}
		}
...
func restrictedSurvivingImage(root map[string]any, floor *acceptanceFloor, probe func([]byte) error) ([]byte, bool) {
	...
	admitted := restrictedTypedLoadDefectiveTargets(floor)
