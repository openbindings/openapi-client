...
func TestConfinement_D15SchemaKeywordAuthorsAndThisEngineDeclines(t *testing.T) {
	...
	// The floor still records D15 at its smallest response projection even
	// though this legacy typed-loader seam declines the raw document.
	...
	if verdict == nil || verdict.Disposition != "represented" {
		t.Fatalf("the reaching operation must remain represented, got %+v", verdict)
	}
	found := false
	for _, defects := range verdict.Projections {
	...
	if !found {
		t.Fatalf("the response projection must name the D15 position, got %+v", verdict.Projections)
	}
}
...
func TestConfinement_PathsReferenceObjectRefusesWithThePart2Reason(t *testing.T) {
	...
	} else if !strings.Contains(err.Error(), "whole-source refusal") {
