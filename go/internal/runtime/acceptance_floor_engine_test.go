...
func TestAcceptanceFloor_EnumerationFiltersInvalidTarget(t *testing.T) {
	...
	if len(operations) != 1 {
		t.Fatalf("expected the unusable target filtered from the invocable inventory, got %d operations", len(operations))
	}
	if operations[0].Ref != "#/paths/~1good/get" {
		t.Errorf("invocable target %q", operations[0].Ref)
	}
}
