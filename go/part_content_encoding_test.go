...
// that package's BUILT dist; changing it in one engine without the others fails
// here.
...
func partContentEncodingDecision(t *testing.T, c partContentEncodingCase) string {
	...
	if _, err := planRequestBodiesFor(doc, item.Post, profileFullCoordinate); err != nil {
		return "refused"
	}
...
func TestRevision3PartContentTransferEncoding(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			reader, contentType, err := buildMultipartBodyForRevision(doc, media, map[string]any{"p": test.value}, profileFullCoordinate)
			if err != nil {
