...
func yamlScalarResolutionImage(t *testing.T, c yamlScalarResolutionCase) string {
	t.Helper()
	document, err := loadDocument(context.Background(), nil, Source{Content: yamlScalarResolutionDocument(c)}, false)
	if err != nil {
...
func TestYAMLScalarResolutionRefusalsAreLoud(t *testing.T) {
	for _, c := range loadYAMLScalarResolutionTable(t) {
		...
		_, err := loadDocument(context.Background(), nil, Source{Content: yamlScalarResolutionDocument(c)}, false)
		if err == nil {
