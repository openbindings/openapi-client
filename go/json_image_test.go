...
// executed by openbindings-go/formats/openapi and by
...
func jsonImageEmission(t *testing.T, c jsonImageCase) string {
	t.Helper()
	doc, err := loadDocument(context.Background(), nil, Source{Content: jsonImageDocument(t, c)}, false)
	if err != nil {
	...
	if c.Lane == "parameter-content" {
		...
		text, err := serializeParamContentFor(op.Parameters[0].Value, c.Value, profileFullCoordinate)
		if err != nil {
			return "error"
		}
		return text
	...
	if c.Lane == "urlencoded-content" || c.Lane == "urlencoded-style" {
		encoded, err := buildURLEncodedBodyForRevision(doc, media, fields, profileFullCoordinate)
		if err != nil {
			return "error"
		}
		return encoded
	...
	reader, contentType, err := buildMultipartBodyForRevision(doc, media, fields, profileFullCoordinate)
	if err != nil {
		return "error"
	}
...
func TestJSONImageRoutingTransformUsesTheConvention(t *testing.T) {
	...
	got := routes.transformExpressionFor(profileFullCoordinate)
...
func TestJSONImageParameterLaneOnTheShippedPath(t *testing.T) {
	...
	routed, err := routeInputFor(op.Parameters, map[string]any{c.PropertyName: c.Value}, "/probe", nil, profileFullCoordinate)
	if err != nil {
