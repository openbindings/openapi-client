...
// executed by openbindings-go/formats/openapi, by openapi-client/typescript,
...
func jsonImageEmission(t *testing.T, c jsonImageCase) string {
	...
	doc, err := loadDocument(context.Background(), nil, Source{Content: encoded}, false)
	if err != nil {
	...
	if c.Lane == "parameter-content" {
		routed, err := routeInputFor(op.Parameters, c.Input, "/probe", nil, profileFullCoordinate)
		if err != nil {
	...
	routed, err := routeInputFor(op.Parameters, c.Input, "/probe", plan, profileFullCoordinate)
	if err != nil {
		t.Fatalf("%s: route input: %v", c.Name, err)
	}
...
func TestJSONImageRoutingTransformEmitsLiteralCharacters(t *testing.T) {
	...
	expression := routes.transformExpressionFor(profileRoutedCoordinate)
