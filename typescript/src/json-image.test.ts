...
import { buildRequestBody, planRequestBodies } from "./media.js";
...
import { OPENAPI_PROFILE_FULL, OPENAPI_PROFILE_ROUTED } from "./profile.js";
import type { BodyPlan } from "./media.js";
import type { OpenAPIDocument, OpenAPIOperation, OpenAPIParameter } from "./types.js";

// The shared case table. The identical file is executed by
// openbindings-go/formats/openapi, openapi-client/go, and
...
/**
 * Run one cell through this engine's own shipped path: the case's WHOLE
 * document is loaded by the shipped loader, the request bodies are planned,
 * the flat input is routed, and the wire body (or, for the parameter lane, the
 * wire header) is rendered.
 */
export async function jsonImageEmission(c: JSONImageCase): Promise<string> {
  ...
  const plan = plans.find((candidate: BodyPlan) => candidate.mediaType === c.media);
...
describe("JSON image case table", () => {
  ...
  // The control, stated as a claim in its own right rather than left implicit
  // in six cells. The urlencoded STYLE path expands an object's members as
  // separate name=value pairs and never forms a JSON image, and the two
  // urlencoded lanes are SUPPOSED to disagree about a space, so no style-lane
  // expectation may carry the JSON image's punctuation or the content path's
  // `+`.
...
describe("the JSON image convention", () => {
  // Pins the convention at the encoder itself, below any lane. RFC 8259 §7
  // permits both spellings; which one this engine emits is its own documented
  // behavior, and it is this.
  it("emits the literal characters", () => {
    expect(jsonImage({ k: "a&b<c>d" })).toBe('{"k":"a&b<c>d"}');
  });

  // The fifth emission site, which is not a wire lane: the revision-2 routing
  // transform expression this engine builds from parameter and body route
  // names.
  it("carries literal characters through the routing transform expression", () => {
    const parameters = [
      { name: "a&b<c>d", in: "query" },
