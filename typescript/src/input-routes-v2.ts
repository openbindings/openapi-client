...
import { OPENAPI_PROFILE_ROUTED, type OpenAPIExecutionProfile } from "./profile.js";
import {
  FAMILY_JSON,
  type BodyPlan,
} from "./media.js";
...
export interface RoutedEnvelope {
  ...
  bodyFields: Record<string, string>;
  wholeBodyField: string;
  /** Native callers can preserve an explicitly supplied empty object/form body. */
  bodyPresent?: boolean;
}
...
export function parseRoutedEnvelope(
  ...
  const revision = profile.mediaFidelity ? "media-fidelity" : "routed";
  ...
  if (envelope.body !== undefined) {
    ...
    if (body.present !== undefined) {
      if (typeof body.present !== "boolean") {
        throw new Error(`${revision} routed body present marker must be boolean`);
      }
      bodyPresent = body.present;
    }
  }
  return { value, parameters, bodyFields, wholeBodyField, bodyPresent };
}
...
export function routeEnvelope(
  ...
  const routed: RoutedInput = {
    ...
    bodyFields: {},
    bodyValue: undefined,
    bodySet: envelope.bodyPresent === true,
