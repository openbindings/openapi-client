...
import { OPENAPI_PROFILE_MEDIA } from "./profile.js";
...
describe("buildMultipartBody", () => {
  ...
  it("carries OAS 3.0 binary parts under their declared non-default content type", async () => {
    ...
    const plan = planRequestBody(
      opWithRequestBody({ "multipart/form-data": media }, true),
      { profile: OPENAPI_PROFILE_MEDIA, openapiVersion: "3.0.3" },
    );
    const wire = buildRequestBody(
      DOC_30,
      plan,
...
// TestRevision3MultipartTypeAbsentPartRefusesOnEveryEdition and
...
describe("§9.2 type-absent and nullable-choice parts", () => {
  ...
  // context-required species. The base planner's own refusal, which the
  // shipped path had been routing around through the raw view, is gone.
  it.each([["3.0.0"], ["3.0.1"], ["3.0.2"], ["3.0.3"], ["3.0.4"]])(
    ...
    (edition) => {
      ...
      expect(() => planRequestBody(
        opWithRequestBody({ "multipart/form-data": media }, true),
        { profile: OPENAPI_PROFILE_MEDIA, openapiVersion: edition },
      )).not.toThrow();
    },
  );

  // The other half, on its own ground: every accepted 3.1 edition STATES
  // application/octet-stream for a part whose `type` is absent -- 3.1.1 and
  // 3.1.2 in the Encoding Object default table's first row, 3.1.0 through the
  // total catch-all closing its prose enumeration -- and this revision defines
  // no JSON-to-octet part boundary. One outcome, two grounds.
  ...
  it("collapses a nullable choice to the non-null branch's carriage and elides null", async () => {
    ...
    const plan = planRequestBody(
      opWithRequestBody({ "multipart/form-data": media }, true),
      { profile: OPENAPI_PROFILE_MEDIA, openapiVersion: "3.1.0" },
    );
  ...
  it("applies the same rules on the urlencoded lane", () => {
    // The nullable-choice collapse is read under the 3.1 line, which is where
    // a {"type": "null"} branch belongs; the type-absent field is read under
    // both lines, which since escalation M2 refuse it alike.
    ...
    expect(() => planRequestBody(
      ...
      { profile: OPENAPI_PROFILE_MEDIA, openapiVersion: "3.1.2" },
    )).not.toThrow();
...
describe("string-carriage lane (declaration-scoped)", () => {
  ...
  it.each([
    ...
    const plans = planRequestBodies(
      opWithRequestBody({ [media as string]: { schema } }, true),
      options,
    );
    expect(plans).toHaveLength(1);
