  ...
  const dereferenced = await dereference<OpenAPIDocument>(normalized as Record<string, unknown>, {
    ...
    onFragmentBelowReference: followsPointerBelowReference(openapiVersion)
      ...
      : ({ reference, token, standingRef }) => {
        throw new Error(
          pointerBelowReferenceRefusal(reference, openapiVersion, token, standingRef),
        );
      },
  });
 ...
 * Read against the document's literal contents that pointer dies at
 * `properties`: `Alias` is an object with exactly one member and it is not
 * `properties`. Read against a document in which references have already been
 * substituted for their targets, the same pointer walks through `Alias` into
 ...
 *   references used in $ref are processed as per JSON Reference, using the URL
 *   of the current document as the base URI." (1 occurrence in each of the
 *   five; 0 in the 3.1 line.) The Reference Object says it a second time: "The
 *   Reference Object is defined by JSON Reference and follows the same
 *   structure, behavior and rules", and "For this specification, reference
 *   resolution is accomplished as defined by the JSON Reference specification
 *   and not by the JSON Schema specification." JSON Reference
 *   (draft-pbryan-zyp-json-ref-03) §1 states its own basis as transclusion,
 *   "the use of a target resource as an effective substitute for the
 *   reference"; §3 "Any members other than "$ref" in a JSON Reference object
 *   SHALL be ignored", so such a node has no other denotable members at all;
 *   §4 "Resolution of a JSON Reference object SHOULD yield the referenced JSON
 *   value. Implementations MAY choose to replace the reference with the
 *   referenced value." Following is deference rung 1.
 *
 * OAS 3.1.0–3.1.2 → REFUSE. §4.6 "Relative References in URIs": "If a URI
 *   contains a fragment identifier, then the fragment should be resolved per
 *   the fragment resolution mechanism of the referenced document. If the
 *   representation of the referenced document is JSON or YAML, then the
 *   fragment identifier SHOULD be interpreted as a JSON-Pointer as per
 *   [RFC6901]." (1 occurrence in each of the three; 0 in all five 3.0
 *   editions. The spelling is the HYPHENATED `JSON-Pointer`, which is why an
 *   earlier audit searching the spaced form read silence here.) The fragment is
 *   therefore an RFC 6901 pointer over the REFERENCED DOCUMENT, and the 3.1
 *   line's Schema Object "is a superset of the JSON Schema Specification Draft
 *   2020-12", whose §8.2.3.1 makes `$ref` "an applicator that is used to
 *   reference a statically identified schema. Its results are the results of
 *   the referenced schema", with the authors' own note that "other keywords can
 *   appear alongside of "$ref" in the same schema object". 2020-12 substitutes
 *   nothing: the referencing node keeps its members, the next token names none
 *   of them, and RFC 6901 §4's error condition arises. No 3.1 edition mentions
 *   JSON Reference at all (0 occurrences in 3.1.1 and 3.1.2; the single 3.1.0
 *   hit is an unrelated `operationRef` escaping note), and no accepted edition
 *   anywhere uses the word "transclusion" or carries a replacement permission
 *   of its own.
 *
 ...
 * informative one.
 */
export function followsPointerBelowReference(openapiVersion: string): boolean {
...
/**
 * The one diagnostic, spelled identically by all three engines so a consumer
 * reads the same sentence whichever one ran it. It names the reference whose
 * fragment was being evaluated, the edition whose text decided the question,
 * the token that identified nothing, and the reference object standing in the
 * pointer's path.
 */
export function pointerBelowReferenceRefusal(
  reference: string,
  openapiVersion: string,
  token: string,
  standingRef: string,
): string {
  return `openapi: reference ${JSON.stringify(reference)} is unresolvable under OAS ${openapiVersion}: `
    + `its fragment's token ${JSON.stringify(token)} identifies no member of the referenced document's `
    + `literal contents, where the reference object ${JSON.stringify(standingRef)} stands in the pointer's `
    + `path; on the 3.1 line a fragment is a JSON-Pointer over the referenced document (OAS §4.6) and `
    + `$ref substitutes nothing (JSON Schema 2020-12 §8.2.3.1, RFC 6901 §4)`;
}
