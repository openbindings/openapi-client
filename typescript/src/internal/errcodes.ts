 ...
 * engine and adapter conventions: useful for stable handling within this
 * product, but not an exhaustive cross-protocol failure vocabulary. (One
 * idiom split: wiring errors — unknown operation/binding/source — throw
 * synchronously in TypeScript but surface as pre-errored handles in Go; the
 * local wiring codes below exist so the Go-emitted values are documented here
 * too.)
 *
