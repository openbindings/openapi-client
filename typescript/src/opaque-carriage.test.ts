...
it("preserves exact values in the default external-resource parser", async () => {
  ...
  expect(stringify(document as Value)).toBe('{"item":{"const":9007199254740993,"default":1e400}}');
});

it("detaches native security-handler metadata without changing values", async () => {
  const doc = parse('{"openapi":"3.1.2","info":{"title":"Exact","version":"1"},"servers":[{"url":"https://example.invalid"}],"components":{"securitySchemes":{"custom":{"type":"http","scheme":"custom","x-id":9007199254740993}}},"paths":{"/x":{"get":{"operationId":"test","security":[{"custom":[]}],"responses":{"204":{"description":"ok"}}}}}}');
  ...
  for (let repeat = 0; repeat < 2; repeat++) {
    const result = await client.call("test", {}, {auth: {custom({scheme}) {
      calls++; expect(stringify(scheme["x-id"] as Value)).toBe("9007199254740993");
